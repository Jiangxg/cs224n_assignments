...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc = data[:truncated_len]
        
        masked_len = int(torch.randint(low=1, high=2*int(truncated_len/4), size=(1,))[0])
        masked_idx = int(torch.randint(low = 0, high = truncated_len - masked_len + 1, size=(1,))[0])
        
        prefix = truncated_doc[:masked_idx]
        suffix = truncated_doc[masked_idx + masked_len:]
        masked_content = truncated_doc[masked_idx : masked_idx + masked_len]
